...
public:
	...
	virtual BaseTextureAccess* InitTexture(uintptr_t texturePtr[], unsigned int width, unsigned int height, int format, CHROMA_SUBSAMPLING chroma_subsampling) = 0;

	//virtual void SafeDeleteTexture() = 0;

	//virtual void ResetBufferIndex() = 0;

	//virtual bool TextureAccessIsReady() = 0;

	//virtual void SetPicIsStrided(bool isStrided) = 0;
